    ...
    def _attach(self, item: Item, position: int) -> None:
        ...
        for customer in self.cols[mid].all():  # could you eliminate this inefficient linear search?
            if customer.next_symbol() == item.rule.lhs:
                ...
                new_item.weight += customer.weight # Don # incement weight for attached rule
                self.cols[position].push(new_item)
...
#        self._weight: Dict[Item, float] = {} # Don # store weight of each item in the agenda 
    ...
    def push(self, item: Item) -> None:
        ...
        if item not in self._index:    # O(1) lookup in hash table
            # Don 
            # I think this is where we want to compare weights of rules that are the same
...
class Item:
    ...
    dot_position: int
    start_position: int
    customer: Item = None # Don
    weight: float = 999999999 # Don # weight of the current dotted rule. starts at 0 for predict. incremented if dot is advanced.
    ...
    def with_dot_advanced(self) -> Item:
        ...
        return Item(rule=self.rule, dot_position=self.dot_position + 1, start_position=self.start_position, weight=self.rule.weight)
