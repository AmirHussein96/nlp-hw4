...
class EarleyChart:
    ...
    def _run_earley(self) -> None:
       ...
       # pdb.set_trace()
        ...
        for i, column in tqdm.tqdm(enumerate(self.cols),
            ...
            # predicted LHSs to avoid re-predicting
            predicted_lhss = []
       ...
       # pdb.set_trace()
        ...
        if position < len(self.tokens) and self.tokens[position] == item.next_symbol():
            ...
            if  (new_item.start_position, str(new_item.rule)) in self.tobe_attached:
                # pdb.set_trace()
                self.tobe_attached[(new_item.start_position, str(new_item.rule))] = self.tobe_attached[(new_item.start_position, str(new_item.rule))]
                #del self.tobe_attached[(new_item.start_position, position, str(new_item.rule))]
    ...
    def _attach(self, item: Item, position: int) -> None:
        ...
        for customer in self.cols[mid].all():  # could you eliminate this inefficient linear search? # this searches for all items in the column
            if customer.next_symbol() == item.rule.lhs:
                ...
                # pdb.set_trace()
                #if position==13 and customer.start_position==0:
                 #   pdb.set_trace()
      ...
      #      pdb.set_trace()
        if not self.grammar.is_nonterminal(child.name):
            ...
            if (child.name, startpos, endpos) not in self.best_attached:
               ...
               #child.parent = parent 
                parent.total_weight+= child.total_weight
                parent.add_children(child)  # Don: maybe we are adding an extra child here, we should first remove the old child?
    ...
    def traverse_test(self, node, position):
        if position == 0:
            return
        print(node.rule)
        print(node.children)
        ...
        for i, child in enumerate(node.children):
            ...
            self.traverse_test(child, position - 1)
